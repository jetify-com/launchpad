...
func (ap *authProvider) FetchToken(ctx context.Context, req *auth.FetchTokenRequest) (rr *auth.FetchTokenResponse, err error) {
	...
	tokenOpts := authutil.TokenOptions{
		Realm:    req.Realm,
		Service:  req.Service,
		Scopes:   req.Scopes,
	...
	if creds.Secret != "" {
		...
		if _, ok := ap.loggerCache[name]; !ok {
			if err := progresswriter.Wrap(name, ap.logger, done); err != nil {
				return nil, errors.WithStack(err)
			}
		}
		ap.mu.Unlock()
		// credential information is provided, use oauth POST endpoint
		resp, err := authutil.FetchTokenWithOAuth(ctx, http.DefaultClient, nil, "buildkit-client", tokenOpts)
		if err != nil {
			...
			if errors.As(err, &errStatus) {
				...
				if (errStatus.StatusCode == 405 && tokenOpts.Username != "") || errStatus.StatusCode == 404 || errStatus.StatusCode == 401 {
					resp, err := authutil.FetchToken(ctx, http.DefaultClient, nil, tokenOpts)
					if err != nil {
						return nil, err
					}
